...
public class CollisionMap {
	...
	public CollisionMap(Sprite image) {
		...
		int width = spriteImage.getWidth(), height = spriteImage.getHeight(), backgroundColor = spriteImage
				.getRGB(0, height - 1);
		collisionMap = new boolean[width][height];
	...
	/**
	 * 
	 * Marks all cells reachable from the starting point along a path of the
	 * same color false in the returned boolean array. This represents a
	 * passable location in the collision map. Used for collision checking.
	 * 
	 * @param image
	 ...
	 * @param y
	 *            y coordinate of the starting locations
	 * @param fillMap
	 *            The boolean map in which to store results. It is passed as a
	 *            parameter so that multiple calls to this function can be made
	 *            in the same boolean array
	 * @return A boolean array of the same dimensions of the image, in which all
	 */
	public static void floodFillGenerate(BufferedImage image, int x, int y,
			boolean[][] fillMap) {
		...
		while (!queue.isEmpty()) {
			...
			if (fillMap[node.x][node.y]) {
				...
				Point w = new Point(node.x, node.y), e = new Point(node.x + 1,
						node.y);
	...
	public boolean collidesWith(int x1, int y1, CollisionMap other, int x2,
			int y2) {
		if (isEmpty || other.isEmpty) {
			return false;
		}
		Rectangle r1 = new Rectangle(x1, y1, collisionMap.length,
				collisionMap[0].length), r2 = new Rectangle(x2, y2,
				other.collisionMap.length, other.collisionMap[0].length);
		if (!r1.intersects(r2)) {
			return false;
		}
		...
		for (int i = r1.x - x1; i < r1.x - x1 + r1.width; i++) { // TODO:
																	// Convince
																	// yourself
																	// that this
																	// always
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		CollisionMap other = (CollisionMap) obj;
